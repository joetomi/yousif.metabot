...
def process_messenger_job(app, msg_details):
    ...
    with app.app_context():
        ...
        enabled = Setting.get("messenger_bot_enabled", "false")
        ...
        processed = ProcessedMessage.query.filter_by(message_id=message_id).first()
        if processed:
        ...
        faqs = MessengerFAQ.query.filter_by(is_active=True).all()
        ...
        gemini_api_key = Setting.get("gemini_api_key", "").strip()
        if not gemini_api_key:
            # If Gemini key is missing, send a default fallback response
            fallback_text = "شكراً لتواصلك معنا. تم استلام رسالتك وسيقوم أحد ممثلي خدمة العملاء بالرد عليك قريباً."
        ...
        tone = Setting.get("messenger_bot_tone", "professional").lower()
        tone_instruction = tone_rules.get(tone, tone_rules["professional"])
        
        knowledge_base = Setting.get("messenger_bot_kb", "").strip()
        if not knowledge_base:
            knowledge_base = "لا تتوفر معلومات تفصيلية عن الشركة حالياً."

        page_name = Setting.get("page_name", "يوسف بوت")
...
"""
            ...
            if response and response.text:
                ...
                reply_text = "شكراً لتواصلك معنا. تم استلام رسالتك وسيقوم أحد ممثلي خدمة العملاء بالرد عليك قريباً."
